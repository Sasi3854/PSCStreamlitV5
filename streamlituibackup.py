    ...
    with risk_home:
        ...
        if run_btn or "analysis_df" in st.session_state:
            ...
            if "risk_table" not in st.session_state:
                st.warning("No analysis results yet. Implement the analysis functions.")
            else:
                risk_table = st.session_state["risk_table"]
                # styled_risk_table = risk_table.style.format_index("{}")
                ...
                with col11:
                    # sel_vessel = st.selectbox("Select Vessel IMO", vessel_ids, format_func=fmt)
                ...
                st.dataframe(styled_view_df, use_container_width=True,column_config={"IMO": st.column_config.TextColumn("IMO")})
